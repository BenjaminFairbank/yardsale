...
RSpec.describe Admin::ItemsController, type: :controller do
  ...
  describe "DELETE#destroy" do
    ...
    it "should not delete the item if the current user is not logged in" do
    ...
    it "should not delete the item if the current user is not an admin" do
      prev_count = Item.count
      sign_in(@user1)
      delete :destroy, params: {id: @item1.id}
      expect(response.status).to eq(302)
    ...
    it "should only delete the item if the current user is an admin" do
